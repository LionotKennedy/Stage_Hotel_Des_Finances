...
export const useGetUser = () => {
    return useQuery('users', async () => {
      ...
      if (!response.ok) {
        console.error('Erreur lors de la récupération des utilisateur:', response.status, response.statusText);
  ...
  export const useGetUserById = (userId) => {
    return useQuery(['users', userId], async () => { // Utilisez une clé de requête unique pour chaque ID
      const token = localStorage.getItem('token');
      console.log('Token:', token); // Debugging token
      
      const response = await fetch(`${API_URL}/edit_user/${userId}`, { // Ajoutez l'ID au chemin de l'URL
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      // Vérifiez si la réponse est OK
      if (!response.ok) {
        console.error('Erreur lors de la récupération du visa:', response.status, response.statusText);
