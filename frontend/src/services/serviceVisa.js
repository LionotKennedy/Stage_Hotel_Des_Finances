...
export const useGetVisa = () => {
    return useQuery('visas', async () => {
      ...
      if (!response.ok) {
        console.error('Erreur lors de la récupération des visa:', response.status, response.statusText);
...
export const useUpdateVisa = () => {
  ...
  export const useDeleteVisa = () => {
    return useMutation(['deleteVisa'], ({ folderId }) => // Correction du paramètre ici
      fetch(`${API_URL}/delete_visa/${folderId}`, { // Corrigez l'URL ici
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`
