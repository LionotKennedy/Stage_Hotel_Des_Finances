...
import { motion, AnimatePresence } from 'framer-motion';
import { useAddFolder, useGetFolderById, useUpdateFolder } from '../../services/serviceFolder';
import { useGetLastFolderNumber } from '../../services/serviceFolder';
import { useSnackbar } from 'notistack';
import { AiOutlineClose } from 'react-icons/ai';
...
export default function CustomModal({ open, handleClose, folderId, mode, onSuccess }) {
  ...
  const { data: lastFolderNumber } = useGetLastFolderNumber();
  ...
  useEffect(() => {
    if (lastFolderNumber) {
      setFields(prevFields => ({
        ...prevFields,
        numero_bordereaux: (parseInt(lastFolderNumber) + 1).toString()
      }));
    }
  }, [lastFolderNumber]);
  ...
  const handleSubmit = async () => {
    ...
    // Validation des champs requis (modifié pour exclure les champs automatiques)
    if (!fields.expiditeur) {
      errors.expiditeur = true;
      hasError = true;
    }
              ...
              <form>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={12}>
                    <TextField
                      name="numero_bordereaux"
                      label="Numéro Bordereaux (Automatique)"
                      variant="standard"
                      fullWidth
                      value={fields.numero_bordereaux}
                      onChange={handleChange}
                  ...
                  <Grid item xs={12} sm={12}>
                    <TextField
                      name="prenom_depose"
                      label="Prénom Déposé (Automatique)"
                      variant="standard"
                      fullWidth
                      value={fields.prenom_depose}
                      onChange={handleChange}
